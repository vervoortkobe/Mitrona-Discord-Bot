...
module.exports.run = async (client, interaction, db) => {
    ...
    if(perms.admin.includes(interaction.member.id) || interaction.member.permissions.has(Discord.PermissionsBitField.Flags.Administrator) || interaction.member.roles.cache.has(serveradminrole) || perms.uncitizen.includes(interaction.member.id)) {
