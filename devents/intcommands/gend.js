...
module.exports.run = async (client, interaction, db) => {
    ...
    if(perms.admin.includes(interaction.member.id) || interaction.member.permissions.has(Discord.PermissionsBitField.Flags.Administrator) || interaction.member.roles.cache.has(serveradminrole) || require("../../checkperms.js").checkPerms(interaction, "gend")) {
      ...
      if(!await giveaways.find().toArray() || await giveaways.find().toArray().length === 0 || !ga) {
        ...
        .setColor(0xff0000)
        .setDescription(`❌ **|** ***Error: I couldn't end that giveaway, because there is no giveaway registered with that messageid!***`)
      ...
      if(ga.giveawaymsg === interaction.options.get("giveaway").value) {
        if(ga.busy === true && ga.time > Date.now()) {
          ...
          .setColor(0xff0000)
          .setDescription(`❌ **|** ***Error: I couldn't end that giveaway, because it has ended already!***`)
